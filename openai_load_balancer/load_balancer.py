...
class LoadBalancer:
    ...
    def send_request(self, endpoint, method_name, **kwargs):
        ...
        openai.api_key = endpoint.api_key_env
        openai.api_type = endpoint.api_type
        openai.model_name = endpoint.model_name
        openai.embedding_model = endpoint.embedding_model
        ...
        if method_name == 'completion_create':
            ...
            kwargs["model"] = openai.embedding_model 
            kwargs["engine"] = openai.embedding_model 
    ...
    def try_send_request(self, method_name, **kwargs):
        ...
        for _ in range(len(self.api_endpoints)):
            ...
            try:
                response = self.send_request(endpoint, method_name, model=model_name, **kwargs)
                print(f"Endpoint {endpoint.base_url} succeeded with")
                # Reset the endpoint on a successful request
                endpoint.reset()
                return response
            except Exception as e:
                # Mark the endpoint as failed
                print(f"Endpoint {endpoint.base_url} failed with error: {e}")
